...
public class Oss {
    ...
    public String upload(MultipartFile file) {
        ...
        OSS oss_client = new OSSClientBuilder().build(endpoint, accessKeyId, accessKeySecret);
        ...
        try {
            ...
            // 上传文件到指定的Bucket
            oss_client.putObject(bucketName, object_name, input_stream);

            // 返回文件的访问URL
            return "https://" + bucketName + "." + endpoint + "/" + object_name;
        } catch (IOException e) {
            log.error("上传失败！");
            return null;
        } finally {
    ...
    public void delete(String url) {
        ...
        try {
            // 删除文件
            oss_client.deleteObject(bucketName, object_name);
        } catch (Exception e) {
            log.error("删除文件出错！");
        } finally {
            // 关闭OSSClient
